...
IPTABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/iptables" IP6TABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/ip6tables" sh "$ROOT/scripts/firewall/firewall-worker.sh" once || fail 'initial ensure failed'
...
IPTABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/iptables" IP6TABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/ip6tables" sh "$ROOT/scripts/firewall/firewall-worker.sh" once || fail 'repeat ensure failed'
...
printf 'state=ready\nmode=1\nnetwork=wifi\nvpn=false\ndns4=192.0.2.1\ndns6=2001:db8::1\n' > "$AGH_STATE_DIR/network.state"
IPTABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/iptables" IP6TABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/ip6tables" sh "$ROOT/scripts/firewall/firewall-worker.sh" once || fail 'mode exception ensure failed'
grep -F -- '192.0.2.53' "$fixture/iptables.log" >/dev/null || fail 'mode exception missing'

echo remove > "$AGH_RUN_DIR/firewall/request"
IPTABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/iptables" IP6TABLES_BIN="$ROOT/tests/fixtures/iptables-recording-bin/ip6tables" sh "$ROOT/scripts/firewall/firewall-worker.sh" once || fail 'remove failed'
