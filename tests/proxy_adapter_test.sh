...
sh "$ROOT/module/scripts/adapters/proxy-worker.sh" once || fail 'proxy adapter failed'
...
sh "$ROOT/module/scripts/adapters/proxy-worker.sh" --clean || fail 'proxy clean failed'
...
EOF
if sh "$ROOT/module/scripts/adapters/proxy-worker.sh" once; then fail 'malformed proxy YAML accepted'; fi
...
EOF
if sh "$ROOT/module/scripts/adapters/proxy-worker.sh" once; then fail 'unknown proxy path accepted'; fi
