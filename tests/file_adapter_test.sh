...
if grep -v '^[[:space:]]*#' "$ROOT/module/targets/file-ad-targets.conf" | grep -E '/data/system/ifw|/databases/|/shared_prefs/' >/dev/null; then fail 'unsafe bundled target found'; fi
if grep -v '^[[:space:]]*#' "$ROOT/module/targets/file-ad-targets.conf" | awk -F'|' '$2 ~ /\/files$/ { found=1 } END { exit found ? 0 : 1 }'; then fail 'application files root bundled'; fi
printf 'ad-image\n' > "$fixture/ad.txt"
