...
if grep -v '^[[:space:]]*#' "$ROOT/module/targets/file-ad-targets.conf" | grep -v '|blocked$' | grep -E '/data/system/ifw|/databases/|/shared_prefs/' >/dev/null; then fail 'unsafe active target found'; fi
if grep -v '^[[:space:]]*#' "$ROOT/module/targets/file-ad-targets.conf" | grep -v '|blocked$' | awk -F'|' '$2 ~ /\/files$/ { found=1 } END { exit found ? 0 : 1 }'; then fail 'application files root active target'; fi
printf 'ad-image\n' > "$fixture/ad.txt"
...
sh "$ROOT/module/scripts/adapters/file-worker.sh" --clean || fail 'file clean failed'
