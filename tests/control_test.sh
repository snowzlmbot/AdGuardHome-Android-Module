...
fixture=$(mktemp -d)
cleanup() {
    AGH_ROOT="$fixture/agh" AGH_RUN_DIR="$fixture/agh/run" MODDIR="$ROOT/module" \
      sh "$ROOT/module/scripts/lifecycle/supervisor.sh" stop >/dev/null 2>&1 || true
    rm -rf "$fixture"
}
trap cleanup EXIT
fail() { printf 'FAIL: %s\n' "$*" >&2; exit 1; }
...
sh "$ROOT/module/scripts/lifecycle/control.sh" start >/dev/null
...
sh "$ROOT/module/scripts/lifecycle/control.sh" pause >/dev/null
...
sh "$ROOT/module/scripts/lifecycle/control.sh" resume >/dev/null
wait_count=0
...
sh "$ROOT/module/scripts/lifecycle/supervisor.sh" stop >/dev/null
wait_count=0
