...
- 支持 Wi‑Fi、移动数据、以太网及其上叠加的 VPN；网络切换无需手动重配。
- 支持 VPN 与 Box/Clash/Mihomo 等代理模块同时使用，代理适配仅在核心就绪时生效。
...
文件级去广告默认关闭。0.1.15 起，模块内置二次维护者清单的安全导入版本，并可在 KernelSU WebUI 中更新规则、按包名扫描或覆盖已安装应用。只要目标路径不存在就跳过，不会创建占位目录；用户后来修改过的目标会标记 warning 并继续处理其他应用。受保护的 IFW、数据库、shared_prefs 和应用 files 根目录仅保留为 blocked 记录，不会被触碰。它不能代替 DNS 过滤。
