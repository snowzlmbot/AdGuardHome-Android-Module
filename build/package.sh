...
if [ -n "${AGH_BINARY_ROOT:-}" ]; then
    for asset_arch in arm64 armv7; do
        local_binary="$AGH_BINARY_ROOT/$asset_arch/AdGuardHome"
        [ -x "$local_binary" ] || { printf 'missing locally built binary: %s\n' "$asset_arch" >&2; exit 1; }
        mkdir -p "$stage/bin/$asset_arch"
        cp "$local_binary" "$stage/bin/$asset_arch/AdGuardHome"
